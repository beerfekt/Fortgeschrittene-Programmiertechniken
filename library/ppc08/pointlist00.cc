...
{
    PointListNode* node = new PointListNode(point); // <> Erzeugen eines neuen Listenknotens mit dem Wert `point`
    ...
    if (mHead) { // <> Allgemeiner Fall: nicht-leere Liste
        ...
        assert(!n->mNext); // <> Zusicherung: `n` hat _keinen_ Nachfolgeknoten
