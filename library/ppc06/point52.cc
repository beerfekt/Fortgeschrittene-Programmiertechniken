...
Point::Point(XCoor x, YCoor y) : mX(x.mValue), mY(y.mValue), mSNR(++mObjectCounter)
{
}

Point::Point(Radius r, Angle phi) : mX(r.mValue * std::cos(phi.mRadiant)), mY(r.mValue * std::sin(phi.mRadiant)), mSNR(++mObjectCounter)
{
}
//[cctor
Point::Point(const Point& other) : // <> Definition des Kopierkonstruktors
    mX(other.mX), mY(other.mY), mColor(other.mColor), // <> Diese Attribute werden kopiert
    mSNR(++mObjectCounter) // <> Die Seriennummer wird _neu_ generiert
{
}
//]
