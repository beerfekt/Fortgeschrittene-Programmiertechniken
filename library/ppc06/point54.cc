...
{
    if (&rhs != this) { // <> Überprüfung auf Selbstzuweisung
        mX = rhs.mX; // <> Zuweisen der non-`const` Datenelemente, die den Zustand bestimmen
        mY = rhs.mY;
        mColor = rhs.mColor;
    }
    return *this; // <> Das eigene Objekt (`this` ist ein _Zeiger_ auf das eigene Objekt) als Ergebnis des Operators
}
//]
