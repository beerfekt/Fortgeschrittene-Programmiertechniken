...
//[ctors
Point::Point(double x, double y, const std::string& color) : // <> Dieser _private_ Konstruktor übernimmt die Initialiserung der Datenelemente. Er ist wegen der unspezifischen `double`-Parameter nur zur _Delegation_ in der Klasse selbst bestimmt.
    mX(x), mY(y), mColor(color),
    mSNR(++mObjectCounter) // <> Der _zentrale_ Algorithmus für die Generierung der Seriennummer
{
}

Point::Point(XCoor x, YCoor y) : Point(x.mValue, y.mValue) // <> Delegation an den _privaten_ Konstruktor
{
}

Point::Point(Radius r, Angle phi) : Point(r.mValue * std::cos(phi.mRadiant), r.mValue * std::sin(phi.mRadiant)) // <> Delegation an den _privaten_ Konstruktor
{
}

Point::Point(const Point& other) : Point(other.mX, other.mY, other.mColor) // <> Delegation an den _privaten_ Konstruktor
{
}
//]
...
{
    assert(r >= 0.0); // <> Sicherstellen eines validen Zustands
}
