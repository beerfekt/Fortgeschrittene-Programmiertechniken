...
//[ctor
Point::Point(XCoor x, YCoor y) :
    mX(x.mValue), mY(y.mValue),
    mSNR(++mObjectCounter) // <> Generierung der Seriennummer
{
}

Point::Point(Radius r, Angle phi) :
    mX(r.mValue * std::cos(phi.mRadiant)), mY(r.mValue * std::sin(phi.mRadiant)),
    mSNR(++mObjectCounter) // <> Generierung der Seriennummer
{
}
//]
