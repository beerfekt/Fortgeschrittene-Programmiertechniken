...
#include "svgfile.h"
#include <iostream>
//[svgfile -headerfooter -trace
#include <cassert> // <> Diese Include-Datei ist notwendig, um Zusicherungen mit Hilfe von `assert` schreiben zu können.
//]
...
//LifeTime SVGFile::mStaticLifeTime;
// Ende Klassenvariablen

//[svgfile
SVGFile::SVGFile(std::string filename) :
    mFile(filename) // <> Initialisierung des Datenelements `mFile` mit dem Dateinamen -> Erzeugen und Öffnen
{
    assert(mFile.is_open()); // <> Sicherstellen des geöffneten Zustands
    //[headerfooter
    mFile << header1 << std::endl; // <> Einfügen des XML-Prologs
...
SVGFile::~SVGFile()
{
    ...
    //[headerfooter
    mFile << footer1 << std::endl; // <> Einfügen des XML-Epilogs
    //]
    //[trace
    std::cout << __PRETTY_FUNCTION__ << std::endl;
    //]
...
void SVGFile::add(std::string svgElement)
{
    ...
    mFile << svgElement << std::endl; // <> Einfügen eines SVG-Strings
}
