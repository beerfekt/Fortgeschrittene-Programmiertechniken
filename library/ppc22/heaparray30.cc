...
SIMPLETEST("iterator1") {
    ...
    for(auto it = ha.begin(); // <> Der Anfang-Iterator wird bestimmt.
        it != ha.end(); // <> Vergleich mit dem Ende-Iterator
        ++it) { // <> Inkrement des Operators
        if (*it != 0) { // <> Dereferenzierung über den Indirektionsoperator
            return false;
        }
    }
