...
namespace PPC {
    namespace detail {
        ...
        //[meta
        template<typename L> struct back_impl; // <> Das allg. Template ist immer Voraussetzung
        template<template<typename, typename...> typename L, typename F, typename... I>
        struct back_impl<L<F, I...>> { // <> Eine partielle Spezialisierung für Typ-Listen
            typedef typename back_impl<L<I...>>::type type; // <> Rekursion über den Rest der Liste
        };
        template<template<typename> typename L, typename B>
        struct back_impl<L<B>> { // <> Eine partielle Spezialisierung für den Fall einer Liste mit _einem_ Element
            typedef B type;
        };
        //]
    }
